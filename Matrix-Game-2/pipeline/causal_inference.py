...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        # ---- profiling/benchmark additions ----
        warmup_blocks: int = 0,
        max_blocks: int = 0,              # 0 => all blocks
        torch_profiler=None,              # optional torch.profiler.profile(...) context object
        return_metrics: bool = False,      # if True, return (videos, metrics)
        ...
        # ---- metrics ----
        use_metrics = bool(profile) or bool(return_metrics)
        metrics = {
            "block_ms_total": [],
            "block_ms_denoise": [],
            "block_ms_ctx": [],
        ...
        # ---- Step 2: cache context feature via initial_latent (unchanged) ----
        ...
        for block_i, current_num_frames in enumerate(tqdm(all_num_frames[:blocks_to_run])):
            ...
            with record_function(f"BLOCK_{block_i:03d}"):
                ...
                if profile_focus in ("all", "denoise", "ctx", "decode"):
                    if use_metrics:
                        ev_den_s.record()
                ...
                if use_metrics:
                    ...
                    if not is_warmup:
                        ...
                        if profile:
                            print(f"block_total_ms: {total_ms:.3f}  denoise_ms: {den_ms:.3f}  ctx_ms: {ctx_ms:.3f}  decode_ms: {dec_ms:.3f}", flush=True)
                            if did_decode and video is not None:
